...
# 3. 데이터 등록 로직 (중괄호를 없애고 에러를 완전히 방지함)
if submit_button:
        ...
        with st.spinner("🌍 위치 좌표를 찾고 구글 시트에 저장하는 중..."):
            try:
                ...
                if geocoded_location:
                    # 💡 중괄호 기호를 쓰지 않고 안전하게 데이터 생성
                    reg_time = datetime.now().strftime("%Y-%m-%d %H:%M")
                    lat_val = float(geocoded_location.latitude)
                    lng_val = float(geocoded_location.longitude)
                    
                    # 딕셔너리 결합 에러를 방지하기 위한 정석적인 방식
                    data_payload = {}
                    data_payload["등록시간"] = reg_time
                    data_payload["이름"] = name
                    data_payload["나이"] = age
                    data_payload["위치"] = location_name
                    data_payload["위도"] = lat_val
                    data_payload["경도"] = lng_val
                    data_payload["특징"] = description
                    ...
                    if response.status_code == 200:
                        ...
                        st.error("❌ 구글 시트 웹앱 연결 실패 (권한 설정을 확인해 주세요)")
                else:
